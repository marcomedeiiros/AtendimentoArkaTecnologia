...
/**
 * BranchEditor - Editor expansível para uma ramificação (opção de menu).
 * Permite editar todos os campos: rótulo, palavras-chave, destino, ação, setor, mensagens.
 */
function BranchEditor({ opcao, index, nodes, onUpdate, onRemove }) {
  ...
  const atualizarCampo = (campo, valor) => {
    onUpdate({ ...opcao, [campo]: valor });
  };

  return (
    <div className="rounded-xl bg-grafite-700 border border-linha space-y-2">
      ...
      <div className="p-2.5">
        <div className="flex items-start justify-between gap-2">
          ...
          >
            {expandido ? <ChevronUp size={14} className="text-slate-400 shrink-0" /> : <ChevronDown size={14} className="text-slate-400 shrink-0" />}
            <div className="flex-1 min-w-0">
              <div className="text-[11px] font-semibold text-white break-words">
                {opcao.rotulo || (opcao.esperaEscolha ? '(sem rótulo)' : 'Qualquer resposta')}
              </div>
              {!expandido && (
                <div className="text-[9px] text-slate-400 mt-0.5 truncate">
                  → {descreverDestino(opcao)}
                </div>
              )}
            </div>
            <Edit2 size={12} className="text-slate-400 group-hover:text-acao-200 transition-colors shrink-0" />
          </button>
          <button
            onClick={onRemove}
        ...
        {!expandido && Array.isArray(opcao.palavrasChave) && opcao.palavrasChave.length > 0 && (
          <div className="flex flex-wrap gap-1 mt-2">
            {opcao.palavrasChave.slice(0, 3).map((p, j) => (
            ...
            {opcao.palavrasChave.length > 3 && (
              <span className="text-[9px] text-slate-500">+{opcao.palavrasChave.length - 3}</span>
            )}
          </div>
        )}
      </div>
      ...
      {/* Conteúdo expansível */}
      {expandido && (
        <div className="px-2.5 pb-2.5 space-y-3 border-t border-linha/50 pt-3">
          {/* Rótulo */}
          <div>
            ...
            <input
              type="text"
              value={opcao.rotulo || ''}
              onChange={(e) => atualizarCampo('rotulo', e.target.value)}
              placeholder="Ex: Suporte Técnico, Financeiro..."
              className="w-full bg-grafite-800 border border-linha rounded-lg px-2.5 py-1.5 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-acao/50"
            />
          </div>
          ...
          <div>
            <div className="flex items-center justify-between text-[10px] text-slate-300 mb-1">
              <label className="font-semibold">Texto do Botão (WhatsApp)</label>
              <span className={`text-[9px] ${String(opcao.botao || '').length > 20 ? 'text-falha-400 font-bold' : 'text-slate-400'}`}>
                {String(opcao.botao || '').length}/20 chars
              </span>
            </div>
            <input
              type="text"
              maxLength={24}
              value={opcao.botao || ''}
              onChange={(e) => atualizarCampo('botao', e.target.value)}
              placeholder={opcao.rotulo?.split(',')[1] || opcao.rotulo?.split(',')[0] || 'Texto do botão'}
              className="w-full bg-grafite-800 border border-linha rounded-lg px-2.5 py-1.5 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-acao/50"
            />
            <p className="text-[9px] text-slate-500 mt-1">Máximo 20 caracteres para boa visualização no WhatsApp</p>
          </div>
          ...
          <div>
            ...
            <input
              type="text"
              value={Array.isArray(opcao.palavrasChave) ? opcao.palavrasChave.join(', ') : ''}
              onChange={(e) => {
                const palavras = e.target.value
                  .split(',')
                  .map(p => p.trim())
                  .filter(p => p.length > 0);
                atualizarCampo('palavrasChave', palavras);
              }}
              placeholder="Ex: 1, tecnico, suporte, ajuda"
              className="w-full bg-grafite-800 border border-linha rounded-lg px-2.5 py-1.5 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-acao/50 font-mono"
            />
            <p className="text-[9px] text-slate-500 mt-1">Cliente digita uma dessas palavras para escolher esta opção</p>
          </div>

          {/* Ação */}
          <div>
            <label className="text-[10px] font-semibold text-slate-300 block mb-1">
              O que acontece quando escolher esta opção
            </label>
            <select
              value={opcao.acao || 'seguir'}
              onChange={(e) => {
                const novaAcao = e.target.value;
                const atualizado = { ...opcao, acao: novaAcao };
                // Limpa campos incompatíveis
                if (novaAcao === 'encerrar') {
                  atualizado.targetId = null;
                  atualizado.filaId = null;
                } else if (novaAcao === 'transferir') {
                  atualizado.targetId = null;
                } else if (novaAcao === 'seguir') {
                  atualizado.filaId = null;
                  atualizado.mensagemHandoff = null;
                  atualizado.mensagemEncerramento = null;
                }
                onUpdate(atualizado);
              }}
              className="w-full bg-grafite-800 border border-linha rounded-lg px-2.5 py-1.5 text-xs text-white focus:outline-none focus:border-acao/50"
            >
              <option value="seguir">Seguir para próximo bloco</option>
              <option value="transferir">Transferir para atendente humano</option>
              <option value="encerrar">Encerrar atendimento</option>
            </select>
          </div>

          {/* Destino (se ação = seguir) */}
          {opcao.acao === 'seguir' && (
            <div>
              <label className="text-[10px] font-semibold text-slate-300 block mb-1">
                Bloco de destino
              ...
              <select
                value={opcao.targetId || ''}
                onChange={(e) => atualizarCampo('targetId', e.target.value || null)}
                className="w-full bg-grafite-800 border border-linha rounded-lg px-2.5 py-1.5 text-xs text-white focus:outline-none focus:border-acao/50"
              >
                <option value="">Selecione o bloco de destino...</option>
                {nodes.filter(n => n.tipo !== 'comentario' && n.tipo !== 'espera').map(n => (
          ...
          {/* Fila (se ação = transferir) */}
          {opcao.acao === 'transferir' && (
            <div>
              <label className="text-[10px] font-semibold text-slate-300 block mb-1">
                Fila de atendimento (opcional)
              </label>
              <input
                type="text"
                value={opcao.filaId || ''}
                onChange={(e) => atualizarCampo('filaId', e.target.value || null)}
                placeholder="ID da fila (deixe vazio para fila padrão)"
                className="w-full bg-grafite-800 border border-linha rounded-lg px-2.5 py-1.5 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-acao/50"
              />
            </div>
          )}

          {/* Mensagem de Handoff (se ação = transferir) */}
          {opcao.acao === 'transferir' && (
            <div>
              <label className="text-[10px] font-semibold text-slate-300 block mb-1">
                Mensagem ao transferir (opcional)
              </label>
              <textarea
                rows={2}
                value={opcao.mensagemHandoff || ''}
                onChange={(e) => atualizarCampo('mensagemHandoff', e.target.value || null)}
                placeholder="Mensagem enviada ao cliente antes de transferir..."
                className="w-full bg-grafite-800 border border-linha rounded-lg px-2.5 py-1.5 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-acao/50 resize-none"
              />
            </div>
          )}

          {/* Mensagem de Encerramento (se ação = encerrar) */}
          {opcao.acao === 'encerrar' && (
            <div>
              <label className="text-[10px] font-semibold text-slate-300 block mb-1">
              ...
              <textarea
                rows={2}
                value={opcao.mensagemEncerramento || ''}
                onChange={(e) => atualizarCampo('mensagemEncerramento', e.target.value || null)}
                placeholder="Mensagem enviada ao cliente antes de encerrar..."
                className="w-full bg-grafite-800 border border-linha rounded-lg px-2.5 py-1.5 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-acao/50 resize-none"
              />
            </div>
          )}
          ...
          <div>
            <label className="text-[10px] font-semibold text-slate-300 block mb-1">
              Definir setor da conversa (opcional)
            </label>
            <input
              type="text"
              value={opcao.setor || ''}
              onChange={(e) => atualizarCampo('setor', e.target.value || null)}
              placeholder="Ex: Técnico, Financeiro, Comercial..."
              className="w-full bg-grafite-800 border border-linha rounded-lg px-2.5 py-1.5 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-acao/50"
            />
            <p className="text-[9px] text-slate-500 mt-1">Define em qual setor esta conversa será categorizada</p>
          </div>
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              type="checkbox"
              checked={opcao.limparCnpj === true}
              onChange={(e) => atualizarCampo('limparCnpj', e.target.checked || null)}
              className="accent-blue-500"
            />
            <span className="text-[10px] text-slate-300">
              Desassociar CNPJ desta conversa e pedir outro
            </span>
          </label>

          {/* Resumo visual */}
          <div className="pt-2 border-t border-linha/50 space-y-1">
            <div className="text-[9px] font-semibold text-slate-400">Resumo:</div>
            <div className="text-[10px] text-slate-300 flex items-center gap-1">
              <span className="text-blue-400">→</span> {descreverDestino(opcao)}
            </div>
            {opcao.setor && (
              <div className="text-[9px] px-1.5 py-0.5 rounded-md bg-blue-500/10 border border-blue-500/30 text-blue-300 inline-block">
                Setor: {opcao.setor}
              </div>
            )}
            {opcao.limparCnpj && (
              <div className="text-[9px] px-1.5 py-0.5 rounded-md bg-espera/10 border border-espera/30 text-espera-400 inline-block ml-1">
                Desassocia CNPJ
              </div>
            )}
          </div>
            ...
            {opcoes.length > 0 && (
              <div className="space-y-2">
                ...
                <p className="text-[10px] text-slate-500 leading-relaxed">
                  Opções deste menu. Clique para expandir e editar cada ramificação completamente.
                </p>
                {opcoes.map((op, i) => (
                  <BranchEditor
                    key={op.id || i}
